...
Server monitoring — catch 5xx errors as they land instead of digging through raw logs after the fact.
Brute-force detection — flag repeated 401/403s from the same source within a short window.
Traffic anomaly spotting — catch bursts of requests from a single source that look like abuse or a misbehaving client.
Rare-event surfacing — flag event types that make up a tiny fraction of overall traffic, which are easy to miss by eye.
Fast triage for small teams — a lightweight, no-infra way to get plain-English incident summaries without standing up a full observability stack.
🏗 System Architecture
text
       [ User ]
          │
...
The user uploads a CSV or clicks Load demo dataset in the frontend.
The FastAPI backend routes the request to ingest.py, which parses and validates each row.
Valid rows are scored by anomaly_detector.py. Rows crossing the threshold are flagged.
Every row is persisted to SQLite; flagged rows also get a row in the anomalies table.
For each flagged row, ai_explainer.py calls Claude with the entry and the specific rule(s) it tripped, and stores the returned explanation.
The frontend polls/fetches this data to render the timeline, with flagged entries highlighted.
🛠 Tech Stack
Backend
